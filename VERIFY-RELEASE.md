...
```sh
sha256sum --check cti-self-hosted-0.1.0-rc.1.zip.sha256
```
...
```powershell
$expected = (Get-Content ./cti-self-hosted-0.1.0-rc.1.zip.sha256).Split()[0]
$actual = (Get-FileHash ./cti-self-hosted-0.1.0-rc.1.zip -Algorithm SHA256).Hash
