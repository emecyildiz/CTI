...
```sh
sha256sum --check cti-self-hosted-0.1.0-rc.2.zip.sha256
```
...
```powershell
$expected = (Get-Content ./cti-self-hosted-0.1.0-rc.2.zip.sha256).Split()[0]
$actual = (Get-FileHash ./cti-self-hosted-0.1.0-rc.2.zip -Algorithm SHA256).Hash
