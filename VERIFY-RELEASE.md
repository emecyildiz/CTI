...
```sh
sha256sum -c cti-self-hosted-0.1.0-rc.9.zip.sha256
```
...
```powershell
$expected = (Get-Content ./cti-self-hosted-0.1.0-rc.9.zip.sha256).Split()[0]
$actual = (Get-FileHash ./cti-self-hosted-0.1.0-rc.9.zip -Algorithm SHA256).Hash
...
```powershell
$expected = (Get-Content ./CTI-Setup-0.1.0-rc.9-win-x64.exe.sha256).Split()[0]
$actual = (Get-FileHash ./CTI-Setup-0.1.0-rc.9-win-x64.exe -Algorithm SHA256).Hash
$actual.Equals($expected, [StringComparison]::OrdinalIgnoreCase)
```
...
```sh
sha256sum -c CTI-Setup-0.1.0-rc.9-linux.sh.sha256
```
